...
plot_combined_mse <- function(mse_data, type) {
  ...
  # 使用factor来确保X轴按照class的顺序排序
  data_to_plot$fam <- factor(data_to_plot$fam, levels = data_to_plot$fam[order(data_to_plot$class)])
  
  p <- ggplot(data_to_plot, aes(x = fam, y = mse, fill = class)) +
