...
# 绘制增强子数据的柱状图
enhancer_plot <- mse_data %>%
  filter(type == "Enhancer") %>%
  ggplot(aes(x = family, y = mse, fill = class)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  facet_wrap(~class, scales = "free_x", labeller = label_both) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 14)) +
  labs(title = "MSE for Enhancer Data by Class and Family",
       x = "Family", y = "MSE")

# 绘制启动子数据的柱状图
promoter_plot <- mse_data %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 14)) +
  labs(title = "MSE for Promoter Data by Class and Family",
       x = "Family", y = "MSE")
