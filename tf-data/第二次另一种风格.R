...
# 定义颜色
colors <- c("Pre-mutation" = "pink", "Post-mutation" = "skyblue")

# 绘制增强子数据的图形
enhancer_plot <- ggplot(enhancer_data, aes(x = Mutation, y = Value, fill = Mutation)) +
...
# 绘制启动子数据的图形
promoter_plot <- ggplot(promoter_data, aes(x = Mutation, y = Value, fill = Mutation)) +
  geom_boxplot(outlier.colour = "white") +
  scale_fill_manual(values = colors) +
  coord_flip() +
  facet_wrap(~Label, scales = "free_y", ncol = 1) +
